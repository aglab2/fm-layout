...
public class SpreadsheetClient
{
    ...
    public RunModel[] GetRuns()
    {
        ...
        var range = $"{_scheduleSheet}!A:Y";
        var request = _sheets.Get(_spreadsheetId, range);
        var response = request.Execute();
        var values = response.Values;

        return SpreadsheetMapper.MapRunsFromRangeData(values).ToArray();
    }

    public RelayRaceModel GetRelayRace()
    {
        if (Service == null)
        {
            return new RelayRaceModel();
        }

        var range = $"{_relayRaceSheet}!A1:F3";
        var request = _sheets.Get(_spreadsheetId, range);
        var response = request.Execute();
        var values = response.Values;
        
        return SpreadsheetMapper.MapRelayRaceFromRangeData(values);
    }
