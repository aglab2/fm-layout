...
public class ObsClient
{
    ...
    private Dictionary<string, List<SceneItemDetails>> _sceneItemsCache = [];
    ...
    public void SendCurrentRunnersImages(string sceneName, string[] currentImages)
    {
        var sceneItems = _sceneItemsCache.ContainsKey(sceneName) ? _sceneItemsCache[sceneName] : _obs.GetSceneItemList(sceneName);
        _sceneItemsCache.TryAdd(sceneName, sceneItems);
        
        var redTeamRenderer = sceneItems.FirstOrDefault(s => 
            s.SourceName.Contains("fm", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("sprite", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("renderer", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("red", StringComparison.InvariantCultureIgnoreCase));
        var yellowTeamRenderer = sceneItems.FirstOrDefault(s => 
            s.SourceName.Contains("fm", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("sprite", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("renderer", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("yellow", StringComparison.InvariantCultureIgnoreCase));
        
        if (yellowTeamRenderer != null)
        {
            _obs.SetInputSettings(yellowTeamRenderer.SourceName, new JObject
            {
        ...
        if (redTeamRenderer != null)
        {
            _obs.SetInputSettings(redTeamRenderer.SourceName, new JObject
            {
                { "websocketUpdate", true },
                { "setCurrentImage", true},
                { "currentImage", currentImages[1] }
            });
        }
    }

    public void SendRelayToScene(string sceneName, RelayRaceModel model, float[] nextX, List<string[]> runnerSprites)
    {
        var sceneItems = _sceneItemsCache.ContainsKey(sceneName) ? _sceneItemsCache[sceneName] : _obs.GetSceneItemList(sceneName);
        _sceneItemsCache.TryAdd(sceneName, sceneItems);
        
        var yellowTeamName = sceneItems.FirstOrDefault(s =>
            s.SourceName.Contains("d_", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("yellow", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("team", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("name", StringComparison.InvariantCultureIgnoreCase));
        var redTeamName = sceneItems.FirstOrDefault(s =>
            s.SourceName.Contains("d_", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("red", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("team", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("name", StringComparison.InvariantCultureIgnoreCase));
        var yellowRunnerName = sceneItems.FirstOrDefault(s =>
            s.SourceName.Contains("d_", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("yellow", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("runner", StringComparison.InvariantCultureIgnoreCase));
        var redRunnerName = sceneItems.FirstOrDefault(s =>
            s.SourceName.Contains("d_", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("red", StringComparison.InvariantCultureIgnoreCase) &&
            s.SourceName.Contains("runner", StringComparison.InvariantCultureIgnoreCase));
        var redTeamRenderer = sceneItems.FirstOrDefault(s => 
        ...
        if (yellowTeamRenderer != null && yellowRunnerName != null)
        {
            _obs.SetInputSettings(yellowTeamRenderer.SourceName, new JObject
            {
                { "sceneName", sceneName },
                { "textSourceName", yellowRunnerName.SourceName },
                { "goToNextPosition", true },
                { "nextPosition", new JObject
                    {
                        {"x", nextX[0]},
                        {"y", 705},
                        {"duration", 2}
                ...
                { "websocketUpdate", true },
                { "allowUpdates", false },
                { "idleImage", runnerSprites[0][0] },
                { "runImage", runnerSprites[0][1] }
            });
        }
        
        if (redTeamRenderer != null && redRunnerName != null)
        {
            _obs.SetInputSettings(redTeamRenderer.SourceName, new JObject
            {
                { "sceneName", sceneName },
                { "textSourceName", redRunnerName.SourceName },
                { "goToNextPosition", true },
                { "nextPosition", new JObject
                    {
                        {"x", nextX[1]},
                ...
                { "websocketUpdate", true },
                { "allowUpdates", false },
                { "idleImage", runnerSprites[1][0] },
                { "runImage", runnerSprites[1][1] }
            });
        }
        
        SetSourceText(yellowTeamName, model.YellowTeamName);
        SetSourceText(redTeamName, model.RedTeamName);
        SetSourceText(yellowRunnerName, model.YellowTeamCurrentPlayer);
        SetSourceText(redRunnerName, model.RedTeamCurrentPlayer);
        
        // HACK: Deadlock prevention due to websocket updating the text source
        if (yellowTeamRenderer != null)
        {
            _obs.SetInputSettings(yellowTeamRenderer.SourceName, new JObject
            {
    ...
    public void SendRunToScene(string sceneName, RunModel model)
    {
        var sceneItems = _sceneItemsCache.ContainsKey(sceneName) ? _sceneItemsCache[sceneName] : _obs.GetSceneItemList(sceneName);
        _sceneItemsCache.TryAdd(sceneName, sceneItems);
        ...
        SetMultipleSourcesText(runnerItems, model.Runners, true);
    ...
    public bool StartStopTimer(string sceneName)
    {
    ...
    public bool PauseContinueTimer(string sceneName)
    {
        var sceneItems = _obs.GetSceneItemList(sceneName);
        var timer = sceneItems.FirstOrDefault(s =>
            s.SourceName.Contains("timer", StringComparison.InvariantCultureIgnoreCase) &&
    ...
    public void CreateLayout(LayoutModel model)
    {
        ...
        _sceneItemsCache.Remove(actualName);
        _sceneItemsCache.Add(actualName, _obs.GetSceneItemList(actualName));
    }
    ...
    private void CreateElement(string sceneName, LayoutElementModel model)
    {
        ...
        var inputList = _obs.GetInputKindList();
        if (!inputList.Contains(model.ObsId))
        {
            return;
        }
    ...
    private void NormalizeLayoutElementSettings(LayoutElementModel model)
    {
        ...
        if (model.Settings.TryGetValue("images", out var imagesJsonArray))
        {
            var imagesArray = (JArray)imagesJsonArray;
            foreach (var element in imagesArray)
            {
                element["file"] = ManifestResourceLoader.GetPathInExe(element["file"]!.ToString());
            }
        }
    ...
    private void SetMultipleSourcesText(IEnumerable<SceneItemDetails> sources, string[] texts, bool extend = false)
    {
        var textIndex = 0;
        var sourcesEnumerated = sources as SceneItemDetails[] ?? sources.ToArray();
        var sourcesAmount = sourcesEnumerated.Length;
        foreach (var source in sourcesEnumerated)
        {
            ...
            var resultText = texts[textIndex++];
            if (textIndex >= sourcesAmount - 1 && extend)
            {
            ...
            SetSourceText(source, resultText);
        }
    }
